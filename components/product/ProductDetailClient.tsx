...
export function ProductDetailClient({ product }: ProductDetailClientProps) {
  ...
  const handleTriggerCompare = () => {
    // 1. Direct subcategory match first (e.g. Bags vs Bags, Knitwear vs Knitwear)
    let altProduct = MASTER_PRODUCTS.find(
      (p) => p.id !== product.id && p.subCategory && p.subCategory === product.subCategory
    );

    // 2. Compatible category match (e.g. apparel/outerwear, acoustics)
    if (!altProduct) {
      if (product.category === 'apparel' || product.category === 'outerwear') {
        altProduct = MASTER_PRODUCTS.find(
          (p) => p.id !== product.id && (p.category === 'apparel' || p.category === 'outerwear')
        );
      } else if (product.category === 'acoustics') {
        altProduct = MASTER_PRODUCTS.find((p) => p.id !== product.id && p.category === 'acoustics');
      } else if (product.category === 'footwear') {
        altProduct = MASTER_PRODUCTS.find((p) => p.id !== product.id && p.category === 'footwear');
      } else {
        altProduct = MASTER_PRODUCTS.find((p) => p.id !== product.id && p.category === product.category);
      }
    }

    // 3. Graceful fallback
    if (!altProduct) {
      altProduct = MASTER_PRODUCTS.find((p) => p.id !== product.id) || MASTER_PRODUCTS[1];
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-start">
          ...
          <div className="lg:col-span-5 space-y-6 lg:sticky lg:top-24 self-start">
            ...
            {/* Luxury Shipping & Quality Guarantees */}
            <div className="grid grid-cols-3 gap-2 py-4 border-y border-white/10 text-center">
              <div className="flex flex-col items-center gap-1 p-2">
                ...
                <span className="text-[10px] font-semibold text-white/90">Express Delivery</span>
                <span className="text-[9px] text-white/50">Next-day on orders &gt;&euro;150</span>
              </div>
              <div className="flex flex-col items-center gap-1 p-2 border-x border-white/10">
                <RotateCcw size={16} className="text-accent-pink" />
                <span className="text-[10px] font-semibold text-white/90">14-Day Returns</span>
                <span className="text-[9px] text-white/50">Complimentary courier pickup</span>
              </div>
              <div className="flex flex-col items-center gap-1 p-2">
                <ShieldCheck size={16} className="text-emerald-400" />
                <span className="text-[10px] font-semibold text-white/90">100% Genuine</span>
                <span className="text-[9px] text-white/50">Atelier certificate verified</span>
              </div>
            </div>

            {/* Accordions (Product Details, Sizing, Care) */}
              ...
              <div className="border border-white/10 rounded-2xl overflow-hidden bg-obsidian-950/40">
                ...
                {openAccordion === 'details' && (
                  <div className="px-4 pb-4 text-xs text-white/60 space-y-2 font-light leading-relaxed border-t border-white/5 pt-3">
                    ...
                    <ul className="list-disc list-inside space-y-1 text-white/50 pt-1">
                      <li>Category: {product.category}</li>
                      <li>Standard European fit</li>
                      <li>Custom dyed hardware accents</li>
                      <li>Certified genuine atelier production</li>
                    </ul>
                  </div>
                )}
              </div>

              {/* Accordion 2: Sizing Info */}
              <div className="border border-white/10 rounded-2xl overflow-hidden bg-obsidian-950/40">
                <button
                  type="button"
                  ...
                  className="w-full p-4 flex items-center justify-between text-xs font-semibold text-white/90 text-left cursor-pointer"
                >
                  <span>Size &amp; Fit Recommendations</span>
                  <ChevronDown
                    size={14}
                    className={`transition-transform duration-200 ${
                      openAccordion === 'sizing' ? 'rotate-180 text-accent-cyan' : ''
                    }`}
                  />
                </button>
