...
export default function OrderSwitcherStrip({
  ...
  // Helper to get concise status tag for active chip
  const getShortStatus = (statusLabel?: string): string => {
    if (!statusLabel) return 'In Transit';
    const clean = statusLabel.trim();
    if (/preparing/i.test(clean)) return 'Preparing';
    if (/out for delivery/i.test(clean)) return 'Out for Delivery';
    if (/delivered/i.test(clean)) return 'Delivered';
    if (/confirmed/i.test(clean)) return 'Confirmed';
    if (/shipped|handed/i.test(clean)) return 'Shipped';
    if (/transit/i.test(clean)) return 'In Transit';
    if (/delayed/i.test(clean)) return 'Delayed';
    if (/cancelled/i.test(clean)) return 'Cancelled';
    if (clean.length <= 15) return clean;
    const parts = clean.split(/[·•\-–—]/);
    return parts[0].trim().slice(0, 15);
  };
  ...
  return (
    <div className="relative z-30 flex items-center justify-between gap-2.5 sm:gap-3 bg-[#08254c]/75 border border-white/15 rounded-xl px-3 sm:px-4 py-2 backdrop-blur-md">
      {/* Left: Active Order Label + Chips Track */}
      <div className="flex items-center gap-2 sm:gap-2.5 flex-1 min-w-0">
        ...
        {/* Scroll-contained chips track */}
        <div
          id="orderSwitcherChips"
          className="flex items-center gap-2 overflow-x-auto no-scrollbar min-w-0 py-0.5"
          ...
          {visibleTopOrders.map((o) => {
            ...
            return (
              <button
                ...
                className={`font-mono text-xs font-semibold px-2.5 sm:px-3 py-1 rounded-full border transition-all inline-flex items-center gap-1.5 flex-shrink-0 ${
                  isActive
        ...
        {/* "+X More" Dropdown Trigger — outside scroll track to prevent menu clipping */}
        {remainingOrders.length > 0 && (
          <div className="relative flex-shrink-0" ref={dropdownRef}>
            ...
            {isDropdownOpen && (
              <div className="absolute right-0 sm:left-0 top-full mt-2 w-72 max-h-72 overflow-y-auto bg-[#08254c] border border-accent-cyan/30 rounded-xl p-1.5 shadow-2xl z-50 backdrop-blur-xl animate-in fade-in slide-in-from-top-1 duration-150 divide-y divide-white/10">
      ...
      {/* Right: Isolated Custom Lookup Action Dock */}
      <div className="flex items-center gap-2 flex-shrink-0 pl-2 border-l border-white/10">
        <button
          type="button"
          onClick={onOpenLookupModal}
          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-accent-cyan/15 hover:bg-accent-cyan/25 border border-accent-cyan/40 text-accent-cyan text-[10.5px] font-bold tracking-wider uppercase transition-all flex-shrink-0 hover:shadow-[0_0_10px_rgba(61,224,255,0.2)] active:scale-[0.98]"
        >
          <Search size={12} />
