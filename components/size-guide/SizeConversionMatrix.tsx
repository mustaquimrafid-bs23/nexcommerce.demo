...
const TROUSER_MATRIX = [
  { eu: 44, usUk: 28, intl: 'XS', waistCm: '72 - 76', waistIn: '28.3 - 29.9', inseamCm: '78 - 80' },
  { eu: 46, usUk: 30, intl: 'S', waistCm: '77 - 80', waistIn: '30.3 - 31.5', inseamCm: '80 - 82' },
  { eu: 48, usUk: 32, intl: 'M', waistCm: '81 - 84', waistIn: '31.9 - 33.1', inseamCm: '82 - 84' },
  { eu: 50, usUk: 34, intl: 'L', waistCm: '85 - 90', waistIn: '33.5 - 35.4', inseamCm: '84 - 86' },
  { eu: 52, usUk: 36, intl: 'XL', waistCm: '91 - 96', waistIn: '35.8 - 37.8', inseamCm: '86 - 88' },
  { eu: 54, usUk: 38, intl: 'XXL', waistCm: '97 - 104', waistIn: '38.2 - 40.9', inseamCm: '88 - 90' },
];

const FOOTWEAR_MATRIX = [
  { eu: 40, uk: 6.5, us: 7.0, insoleCm: 26.0 },
  { eu: 41, uk: 7.5, us: 8.0, insoleCm: 26.7 },
  { eu: 42, uk: 8.5, us: 9.0, insoleCm: 27.3 },
  { eu: 43, uk: 9.5, us: 10.0, insoleCm: 28.0 },
  { eu: 44, uk: 10.0, us: 10.5, insoleCm: 28.7 },
  { eu: 45, uk: 11.0, us: 11.5, insoleCm: 29.3 },
  { eu: 46, uk: 11.5, us: 12.0, insoleCm: 30.0 },
];
  ...
  return (
    <div className="space-y-6 rounded-3xl bg-surface-card border border-white/10 p-6 sm:p-8 shadow-2xl">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-4">
        ...
        {/* Tab switcher */}
        <div className="flex items-center p-1 rounded-xl bg-obsidian-950 border border-white/10 text-xs overflow-x-auto">
          <button
            type="button"
        ...
        {tab === 'trousers' && (
          <table className="w-full text-left border-collapse text-xs">
            <thead>
              <tr className="border-b border-white/10 text-white/50 uppercase tracking-wider font-semibold">
                <th className="py-3 px-4">European (EU)</th>
                <th className="py-3 px-4">UK / US Waist</th>
                <th className="py-3 px-4">International</th>
                <th className="py-3 px-4">Waist (CM)</th>
                <th className="py-3 px-4">Waist (IN)</th>
                <th className="py-3 px-4">Inside Leg (CM)</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/5 font-mono">
              {TROUSER_MATRIX.map((row) => (
                <tr key={row.eu} className="hover:bg-white/[0.02] transition-colors">
                  <td className="py-3 px-4 font-bold text-accent-cyan">EU {row.eu}</td>
                  <td className="py-3 px-4 text-white/90">{row.usUk}&quot;</td>
                  <td className="py-3 px-4 font-sans font-semibold text-accent-pink">{row.intl}</td>
                  <td className="py-3 px-4 text-white/70">{row.waistCm}</td>
                  <td className="py-3 px-4 text-white/70">{row.waistIn}</td>
                  <td className="py-3 px-4 text-white/70">{row.inseamCm}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <table className="w-full text-left border-collapse text-xs">
            ...
            <tbody className="divide-y divide-white/5 font-mono">
              {FOOTWEAR_MATRIX.map((row) => (
                <tr key={row.eu} className="hover:bg-white/[0.02] transition-colors">
                  <td className="py-3 px-4 font-bold text-accent-cyan">EU {row.eu}</td>
                  <td className="py-3 px-4 text-white/90">UK {row.uk}</td>
                  <td className="py-3 px-4 text-white/90">US {row.us}</td>
                  <td className="py-3 px-4 text-accent-pink">{row.insoleCm} cm</td>
                </tr>
              ))}
            </tbody>
          </table>
