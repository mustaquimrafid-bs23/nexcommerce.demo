...
targetFiles.forEach(rel => {
  ...
  // 1. Remove existing top-announcement-bar, site-header, and mobile-nav-drawer
  // We'll replace the block from top-announcement-bar or site-header up to the start of <main or first content container
  ...
  // Pattern match existing header stack
  ...
  if (regexHeaderStack.test(html)) {
    ...
    console.log(`✅ [${rel}] Replaced header stack cleanly.`);
  } else {
    // If not matching stack directly, find where <main starts or after preloader
    console.log(`⚠️ [${rel}] Header stack didn't match single regex, inspecting...`);
  }
});
